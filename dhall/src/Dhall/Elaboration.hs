...
import qualified Dhall.Presyntax as P
...
-- -- | Infer type for a root 'Raw' expression, with setting the current directory.
-- inferRoot :: FilePath -> Raw -> IO (Core, Val)
-- inferRoot path t = runReaderT (infer emptyCxt t) =<< rootState path

-- -- | Check a root 'Raw' expression, with setting the current directory.
-- checkRoot :: FilePath -> Raw -> Val -> IO Core
-- checkRoot path t a = runReaderT (check emptyCxt t a Nothing) =<< rootState path

-- -- | Infer type for a raw expression.
-- inferExpr :: Raw -> IO (Core, Val)
-- inferExpr = inferRoot "."

-- -- | Check type of a raw expression.
-- checkExpr :: Raw -> Val -> IO Core
-- checkExpr = checkRoot "."
