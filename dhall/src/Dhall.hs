...
import Control.Applicative (empty, liftA2, (<|>), Alternative, Const)
...
import Dhall.Core (Expr(..), Chunks(..), coerceEmbed)
import Dhall.Elaboration (infer)
import Dhall.Eval (Core, Nf, Raw, nfEmpty, conv, eval, Env(..))
import Dhall.Util (_ERROR, insert, throws, internalError)
import Dhall.Context (rootState, emptyCxt)
import Dhall.Lint (denote, assertNoImports)
import GHC.Generics
import Lens.Family (LensLike')
import Numeric.Natural (Natural)
import Prelude hiding (maybe, sequence)
import Dhall.Errors (DetailedContextualError(..))
...
data InvalidType = InvalidType
  { invalidTypeExpected   :: !Core
  , invalidTypeExpression :: !Core
  }
  deriving (Typeable)
...
inputWithSettings InputSettings{..} Type{..} txt = do
    expr  <- throws (Dhall.Parser.exprFromText _sourceName txt)
    state <- rootState _rootDirectory
    expr  <- fst <$> runReaderT (infer emptyCxt expr) state
    case extract (nfEmpty expr) of
        Just x  -> pure x
        Nothing -> throwM (InvalidType expected expr)
...
inputExpr = inputExprWithSettings defaultInputSettings
...
inputExprWithSettings InputSettings{..} txt = do
    expr  <- throws (Dhall.Parser.exprFromText _sourceName txt)
    state <- rootState _rootDirectory
    nfEmpty . fst <$> runReaderT (infer emptyCxt expr) state
...
rawInput (Type {..}) expr = do
    case extract $ nfEmpty $ denote $ assertNoImports expr of
        Just x  -> pure x
        Nothing -> empty
...
detailed :: IO a -> IO a
detailed =
  Control.Exception.handle (Control.Exception.throwIO . DetailedContextualError)
...
unsafeExpectUnion
    :: Text -> Core -> Dhall.Map.Map Text (Maybe Core)
...
newtype RecordType a = RecordType
  (Product (Const (Dhall.Map.Map Text Core))
           (Compose ((->) Nf) Maybe)
           a)
...
record (RecordType (Pair (Control.Applicative.Const fields) (Compose extractF))) =
  Type extractF (Record fields)
...
union :: forall a. UnionType a -> Type a
union (UnionType (Compose as)) = Type
    { extract  = extractF
    , expected = Union expect
    }
  where
    expect = (possible . Dhall.expected) <$> as

    extractF :: Nf -> Maybe a
    extractF e0 =
      case e0 of
        UnionLit fld e1 rest -> do
          a <- Dhall.Map.lookup fld as
          guard $ conv Empty (eval Empty (Union ((coerceEmbed <$>) <$> rest)))
                             (eval Empty (Union (Dhall.Map.delete fld expect)))
          extract a e1
        Field (Union _) _ ->
          error "union: enum constructors not yet implemented"
        App (Field (Union _) _) _ ->
          error "union: enum constructors not yet implemented"
        _ -> empty
...
constructor key valueType = UnionType
    ( Compose (Dhall.Map.singleton key valueType) )
...
UnionInputType (Pair (Control.Applicative.Const mx) (Op fx))
    >|< UnionInputType (Pair (Control.Applicative.Const my) (Op fy)) =
    UnionInputType
      (Pair
...
inputUnion (UnionInputType (Pair (Control.Applicative.Const fields) (Op embedF))) =
    InputType
      { embed = \x ->
          let (name, y) = embedF x
